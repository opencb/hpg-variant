...
int read_split_configuration(const char *filename, split_options_t *options, shared_options_t *shared_options) {
    ...
    ret_code = config_lookup_int(config, "split.batch-lines", shared_options->batch_lines->ival);
    ret_code |= config_lookup_int(config, "split.batch-bytes", shared_options->batch_bytes->ival);
    if (ret_code == CONFIG_FALSE) {
...
//     tool_options[6] = shared_options->species;
    ...
    tool_options[11] = split_options->criterion;
